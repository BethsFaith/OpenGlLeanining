...
//         Shaders::getPath(Shaders::Sources::STATIC_LIGHT_FRAG));
    _lighting_shader_program = std::make_shared<ShaderProgram>(
        Shaders::getPath(Shaders::Sources::THIRDD_LIGHT_TEXT_VERT),
        Shaders::getPath(Shaders::Sources::SEARCHLIGHT_FRAG));
...
void Program7::updateView() {
    ...
    glm::vec3 ambient_color = light_color * glm::vec3(0.1f); // 0.2f
    ...
    // свойства света
//    _lighting_shader_program->setInt("light.type", SEARCHLIGHT);
    _lighting_shader_program->set3FloatVector("light.position",_camera->getPosition());
    _lighting_shader_program->set3FloatVector("light.direction",_camera->getFront());
    ...
    _lighting_shader_program->setFloat("light.constant", 1.0f);
    _lighting_shader_program->setFloat("light.linear", 0.09f);
    _lighting_shader_program->setFloat("light.quadratic", 0.032f);
    _lighting_shader_program->setFloat("light.cutOff", glm::cos(glm::radians(12.5f)));
    _lighting_shader_program->setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));
