...
//    auto vbo = std::make_shared<VBO>(coords);
    ...
    auto cube = std::make_shared<Cube>(_lighting_shader_program, coords);
    auto light_cube = std::make_shared<Cube>(_light_source_shader_program, coords);

    light_cube->bind(Settings{.bind_flag = GL_STATIC_DRAW, .with_texture = false, .with_color = false});
    cube->bind(Settings{.bind_flag = GL_STATIC_DRAW, .with_texture = false, .with_color = false});
    ...
    glm::vec3 light_color = {0.45, 0.73, 0.74};
